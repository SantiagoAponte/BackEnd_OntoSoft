        ...
        public class Manager : IRequestHandler<getClinicHistoryInPdf, Stream>
        {
            ...
            public async Task<Stream> Handle(getClinicHistoryInPdf request, CancellationToken cancellationToken)
            {
              ...
              var clinicHistory = await _context.clinicHistories.Where(a => a.UserId == request.Id)
                ...
                throw new ManagerError(HttpStatusCode.BadRequest, new {mensaje = "¡Error! " + "no se encuentra el Id del usuario para generar el pdf"});
                }
                ...
                Console.WriteLine(odontogramData);
                /*relaciones*/
               ...
               for(int i = 0; i < treamentData.Count; i++){
                ...
                PdfPTable tableInfoOdontogram = new PdfPTable(3);
                float[] widths20 = new float[] {60f, 60f, 60f};
                ...
                PdfPTable tableOdontogramText = new PdfPTable(3);
                float[] widths21 = new float[] {60f, 60f, 60f};
               ...
               for(int i = 0; i < odontogramData.Count; i++){
                foreach (var atribute in clinicHistory)
                {
                var name = atribute.Odontogram.ToList();
